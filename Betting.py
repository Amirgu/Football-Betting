...
class Betting:
    """""
    Class for Betting Stategies :(Level Stakes Betting),
    (KellyBetting with increasing stakes),
    (Kelly Betting with unit stakes ),
    To be implemented :
    - Risk minimization,
    - Optimisation over multiples Stackes
    """ ""
    ...
    def kelly_criterion(self, win_prob, odds):
        ...
        odds: Bookie odds
        """
        kelly_f = (odds * win_prob - 1.0) / (odds - 1.0)
    ...
    def kelly_betting(self, proba, y, Over_cote, Under_cote):
        ...
        n0 = len(L)  # Number of eligible bets
        ...
        self.stack_kelly = (Profit - Loss) * self.stack_kelly + (
            1 - s
        ) * self.stack_kelly
    ...
    def level_stakes_betting_unit(self, prob, y, over, under, treshold):
        prob_under = prob[0]
        prob_over = prob[1]
        if prob_over > 1 / over:
            if y == 1:
                self.stack_kelly_unit += over - 1
            else:
                self.stack_kelly_unit += -1
        elif prob_under > 1 / under:
            if y == 0:
                self.stack_kelly_unit += under - 1
            else:
                self.stack_kelly_unit += -1
        self.number_of_bets += 1
        return self.stack_kelly_unit

    def level_stakes_betting_list(self, prob, y, over, under, L, treshold):
        """""
        ...
        n = len(y)
        for i in range(n):
            b = self.level_stakes_betting_unit(
                prob[i], y[i], over[i], under[i], treshold
            )
            L.append(b)
    ...
    def kellybetting_unitlist(self, prob, y, over, under, L, treshold):
        """""
        Same Function as above but for multiple bets
