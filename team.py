...
def UpdateGamei(Lambda,phi1,phi2,team_names,i,teams,dataset,mode='G'):
    """"" 
    ...
    mode: mode of the measure of performance used:'G' if goals scored by either teams, 
    ...
    if mode == 'G':
        ...
        Sh=int(dataset[i]['HST'])+int(dataset[i]['HC'])/3   #### Corners + Shots Home
        Sa=int(dataset[i]['AST'] )+int(dataset[i]['AC'] )/3   #### Corners + Shots Away

    elif mode == 'G+S+C':

        Sh=1.2*int(dataset[i]['FTHG'] ) + int(dataset[i]['HST'])/4 + int(dataset[i]['HC'])/5       
        Sa=1.2*int(dataset[i]['FTAG'] ) + int(dataset[i]['AST'] )/4 + int(dataset[i]['AC'] )/5
    
    jh=int([k for k in range(1,21) if team_names[k]== Home ][0]) -1   ##index of the home team
    
    ja=int([k for k in range(1,21) if team_names[k]== Away ][0])-1    ## index of the away team
    ...
    a_new= max( a + Lambda*phi1*(Sh-0.5*( a + y ) )  ,0)                ##### HA

    c_new= max( c + Lambda*(1-phi1)*(Sh-0.5*( a + y ) )  ,0)           ###### AA

    b_new= max( b + Lambda*phi1*(Sa-0.5*( x + b ) )  ,0)               ####  HD

    d_new= max( d + Lambda*(1-phi1)*(Sa-0.5*( x + b ) )  ,0)            #### AD
    
    #### AWAY UPDATES :

    x_new= max( x + Lambda*phi2*(Sa-0.5*( x + b ) )  ,0)                #### AA

    u_new= max( u + Lambda*(1-phi2)*(Sa-0.5*( x + b ) )  ,0)            #### HA

    y_new= max( y + Lambda*phi2*(Sh-0.5*( a + y ) )  ,0)                #### AD

    v_new= max( v + Lambda*(1-phi2)*(Sh-0.5*( a + y ) )  ,0)            #### HD
